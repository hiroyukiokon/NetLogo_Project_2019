globals
[
  red-count            ; population of red turtles; the value is given in the plot-pens in Plot in the Interface
  blue-count           ; population of blue turtles ; the value is given in the plot-pens in Plot in the Interface
]
...
to wander  ;; turtle procedure
  rt random-float 30 - random-float 30  ; right 30 - random-float 60
  fd 1
end
...
to reproduce
  ...
  [
    ...
    ifelse (random-float 100) < (100 * fertility-remainder)
      [ hatch fertility + 1 [ wander ]] ; by "hatch" primitive, the turtle creates number new turtles.
                                        ; Each new turtle inherits of all its variables,
                                        ; including its location, from its parent.
...
to grim-reaper
  ...
  if num-turtles <= carrying-capacity
    [ stop ]
  let chance-to-die (num-turtles - carrying-capacity) / num-turtles ; exceeding rate of total population over the capacity
  ask turtles
  [
