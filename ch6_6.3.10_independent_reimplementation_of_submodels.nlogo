globals [ q ] ;; q is a parameter to to assaighned  some value

patches-own [ elevation ] ;; a state variable of patches
...
to setup
  ...
  set q 0.3 ;; q is global variable and parameter


  if file-exists? "TestOutput3.csv"
        [file-delete "TestOutput3.csv"]

  file-open "TestOutput3.csv"
...
to move ; The butterfly 'move' procedure, which is in turtle context
  ...
  ; now, save the random number in a local variable
  ; so we can write it out
  let rand-num random-float 1



  ifelse rand-num < q   ;; a conditon whthere random-num is smaller than q?
    ;;[uphill elevation]     ;; move deterministically uphii  ;; an example of misunderstanding primitive "uphill"
    [move-to max-one-of neighbors [elevation]]
    [move-to one-of neighbors]  ;; otherwise randomly

  ; now write out the random number and q .
  ; then the elevation the turtle moved to,
