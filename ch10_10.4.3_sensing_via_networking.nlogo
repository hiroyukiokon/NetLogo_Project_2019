globals [
         number-of-investors
         T                      ;; T is investor's time horizon to the future
                                ;; to be set 5 years
         sensing-radius
         number-of-links        ;; a turtle , investor, have links to other turtles randomly chosen
...
patches-own [ P F ] ;; P represents the annual net profit in money units
                    ;; F is the annual risk of failing business and losing all wealth
...
;; first, delete it instead of appending to it
 if (file-exists? "ch10_Business_Investor_Output.csv")
   [
     carefully
        [file-delete "ch10_Business_Investor_Output.csv"]
        [print error-message]
   ]

  file-open "ch10_Business_Investor_Output.csv"
  file-type "id,"
  file-type "tick,"
  file-type "wealth,"
  file-type "profit,"
...
to go
   ask patches
  [
    set P 1000 + random 9000
    set F 0.01 + random-float 0.1
  ]

  ask turtles
  [
    investor-repositioning  ; a turtle procedure

    accounting

  ]

  update-outputs; writing the valules of variables of each agent at each tick
...
to investor-repositioning ; a turtle procedure for investment actions


  ; creating the potential destination(= all the empty neighbor patches + current one)
  ...
  ;First identify potential destination patches - withiin the sensing-radius.
  ...
  ; Now again, add neighbors of linked turtles
...
to-report utility-for [a-turtle] ; this is in a patch context.
  ; A patch-context reporter that calculate utility
  ; for turtle "a-turtle" in this patch
  ; first get the turtle's current wealth
  let turtles-wealth [W] of a-turtle

  ; Then calculate turtle's utility given its wealth, W,
  ; and relevant patch variables, P and F
  let utility (turtles-wealth + T * P) * ((1 - F) ^ T) ; U = (W + TP)(1 - F)^T
...
"" ""
PENS
"default" 1.0 0 -10141563 true "" "plot mean [W] of turtles"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean [F] of patches with [any? turtles-here]"
...
"" ""
PENS
"default" 1.0 0 -2674135 true "" "plot mean [P] of patches with [any? turtles-here]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count turtles with [W = 0]"
